...
typedef struct btl_usnic_opal_if_t {
...
static int btl_usnic_opal_ifinit(void) 
{
#ifndef __WINDOWS__
    int flag;
    ...
    OBJ_CONSTRUCT(&btl_usnic_opal_if_list, opal_list_t);
    ...
    /* configure using getifaddrs(3) */
    {
        ...
        if (getifaddrs(ifadd_list) < 0) {
            opal_output(0, "btl_usnic_opal_ifinit: getifaddrs() failed with error=%d\n",
                    errno);
            return OPAL_ERROR;
        }
		...
		continue;
            ...
            intf = OBJ_NEW(btl_usnic_opal_if_t);
            if (NULL == intf) {
                opal_output(0, "btl_usnic_opal_ifinit: unable to allocate %lu bytes\n",
                            (int) sizeof(btl_usnic_opal_if_t));
                return OPAL_ERR_OUT_OF_RESOURCE;
            }

            /* fill values into the btl_usnic_opal_if_t */
	    ...
	    intf->if_mask = prefix( sin_addr->sin_addr.s_addr);
            ...
            /* Do not [yet] support getting the MAC address from these
               platforms */
    ...
    /* Beginning of !(various flavors of BSD) */
    {
        ...
        if ((sd = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
            opal_output(0, "btl_usnic_opal_ifinit: socket() failed with errno=%d\n", 
                        errno);
            return OPAL_ERROR;
        }
        ...
        do {
            ...
            if (ioctl(sd, SIOCGIFCONF, &ifconf) < 0) {
                ...
                if (errno != EINVAL && lastlen != 0) {
                    opal_output(0, "btl_usnic_opal_ifinit: ioctl(SIOCGIFCONF) \
                            failed with errno=%d", 
                                errno);
                    free(ifconf.ifc_req);
                    close(sd);
        ...
        if (!successful_locate) {
            opal_output(0, "btl_usnic_opal_ifinit: unable to find network interfaces.");
        ...
        while (rem > 0) {
            ...
            intf = OBJ_NEW(btl_usnic_opal_if_t);
            if (NULL == intf) {
                opal_output(0, "btl_usnic_opal_ifinit: unable to allocated %lu bytes\n", (unsigned long)sizeof(btl_usnic_opal_if_t));
                free(ifconf.ifc_req);
                close(sd);
                return OPAL_ERR_OUT_OF_RESOURCE;
            }
                     ...
                     /proc/net/if_inet6 instead */
    {
        ...
        if ((f = fopen("/proc/net/if_inet6", "r"))) {
            ...
            while (fscanf(f, "%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x%2x %x %x %x %x %20s\n",
                ...
                /* now construct the btl_usnic_opal_if_t */
                ...
                intf->if_mask = pfxlen;
                if (OPAL_SUCCESS == btl_usnic_opal_ifindextoflags(btl_usnic_opal_ifnametoindex (ifname), &flag)) {
                    intf->if_flags = flag;
                } else {
...
/*           || defined(__linux__)  */
    ...
    /* Solaris IPv6 interface discovery */
    {
        ...
        if (sd < 0) {
            opal_output (0, "btl_usnic_opal_ifinit: unable to open IPv6 socket\n");
            return OPAL_ERROR;
        }
        ...
        if (error < 0) {
            opal_output (0,
              "btl_usnic_opal_ifinit: ioctl SIOCGLIFNUM failed with errno=%d\n", errno);
            return OPAL_ERROR;
        }
        ...
        if (NULL == lifconf.lifc_buf) {
            opal_output (0, "btl_usnic_opal_ifinit: IPv6 discovery: malloc() failed\n");
            return OPAL_ERR_OUT_OF_RESOURCE;
        }
        ...
        if (error < 0) {
            opal_output (0,
              "btl_usnic_opal_ifinit: IPv6 SIOCGLIFCONF failed with errno=%d\n", errno);
        }
        ...
        for (i = 0; i + sizeof (struct lifreq) <= lifconf.lifc_len;
            ...
            if (error < 0) {
                opal_output (0,
                  "btl_usnic_opal_ifinit: SIOCGLIFINDEX failed with errno=%d\n", errno);
                return OPAL_ERROR;
            }
            ...
            if (error < 0) {
                opal_output (0,
                  "btl_usnic_opal_ifinit: SIOCGLIFFLAGS failed with errno=%d\n", errno);
                return OPAL_ERROR;
            }
            ...
            if (AF_INET6 == lifreq->lifr_addr.ss_family) {
                ...
                */
                if ( (! IN6_IS_ADDR_LOOPBACK (&my_addr->sin6_addr)) &&
                    ...
                    if (NULL == intf) {
                        opal_output (0,
                                     "btl_usnic_opal_ifinit: unable to allocate %d bytes\n",
                                     sizeof (btl_usnic_opal_if_t));
                        return OPAL_ERR_OUT_OF_RESOURCE;
                    }
...
#else /* __WINDOWS__ implementation begins */
    {
        /* 
           ...
           3. construct btl_usnic_opal_if_list and populate it with the list of interfaces we have
        ...
        if (sd == SOCKET_ERROR) {
            opal_output(0, "btl_usnic_opal_ifinit: WSASocket failed with errno=%d\n",WSAGetLastError());
            return OPAL_ERROR;
        }
        ...
        if (SOCKET_ERROR == WSAIoctl (sd, 
                                      ...
                                      0,
                                    0)) {
            opal_output(0, "btl_usnic_opal_ifinit: WSAIoctl failed with errno=%d\n",WSAGetLastError());
            return OPAL_ERROR;
        }

        /* create and populate btl_usnic_opal_if_list */
        OBJ_CONSTRUCT (&btl_usnic_opal_if_list, opal_list_t);
        ...
        for (i = 0; i < num_interfaces; ++i) {
            ...
            if (0 != (if_list[i].iiFlags & IFF_UP)
                ...
                intf = OBJ_NEW(btl_usnic_opal_if_t);
                if (NULL == intf) {
                    opal_output (0,"btl_usnic_opal_ifinit: Unable to malloc %d bytes",sizeof(opal_list_t));
                    return OPAL_ERR_OUT_OF_RESOURCE;
                }
...
int btl_usnic_opal_iffinalize(void) 
{
    if (already_done) {
#ifndef __WINDOWS__
        btl_usnic_opal_if_t *intf_ptr;
    
        while (NULL != 
               (intf_ptr = (btl_usnic_opal_if_t*)opal_list_remove_first(&btl_usnic_opal_if_list))) {
            OBJ_RELEASE(intf_ptr);
        }
        OBJ_DESTRUCT(&btl_usnic_opal_if_list);
#endif
        already_done = false;
    }
    return OPAL_SUCCESS;
...
int btl_usnic_opal_ifnametoaddr(const char* if_name, struct sockaddr* addr, int length)
{
    btl_usnic_opal_if_t* intf;
...
int btl_usnic_opal_ifnametoindex(const char* if_name)
{
    btl_usnic_opal_if_t* intf;
    int rc = btl_usnic_opal_ifinit();
    if (rc != OPAL_SUCCESS)
...
int16_t btl_usnic_opal_ifnametokindex(const char* if_name)
{
    btl_usnic_opal_if_t* intf;
    int rc = btl_usnic_opal_ifinit();
    if (rc != OPAL_SUCCESS)
...
int btl_usnic_opal_ifindextokindex(int if_index)
{
    btl_usnic_opal_if_t* intf;
    int rc = btl_usnic_opal_ifinit();
    if (rc != OPAL_SUCCESS)
...
int btl_usnic_opal_ifaddrtoname(const char* if_addr, char* if_name, int length)
{
    btl_usnic_opal_if_t* intf;
    int rc;
#if OPAL_WANT_IPV6
...
int btl_usnic_opal_ifcount(void)
{
    if (btl_usnic_opal_ifinit() != OPAL_SUCCESS)
        return (-1);
    return opal_list_get_size(&btl_usnic_opal_if_list);
}
...
int btl_usnic_opal_ifbegin(void)
{
    btl_usnic_opal_if_t *intf;
    if (btl_usnic_opal_ifinit() != OPAL_SUCCESS)
        return (-1);
    intf = (btl_usnic_opal_if_t*)opal_list_get_first(&btl_usnic_opal_if_list);
    if (NULL != intf)
        return intf->if_index;
    return (-1);
}
...
int btl_usnic_opal_ifnext(int if_index)
{
    btl_usnic_opal_if_t *intf;
    if (btl_usnic_opal_ifinit() != OPAL_SUCCESS)
        return (-1);
    ...
    for (intf =  (btl_usnic_opal_if_t*)opal_list_get_first(&btl_usnic_opal_if_list);
        ...
        if (intf->if_index == if_index) {
            do {
                btl_usnic_opal_if_t* if_next = (btl_usnic_opal_if_t*)opal_list_get_next(intf);
                btl_usnic_opal_if_t* if_end =  (btl_usnic_opal_if_t*)opal_list_get_end(&btl_usnic_opal_if_list);
                if (if_next == if_end) {
                    return -1;
                }
                intf = if_next;
...
int btl_usnic_opal_ifindextoaddr(int if_index, struct sockaddr* if_addr, unsigned int length)
{
    btl_usnic_opal_if_t* intf;
    int rc = btl_usnic_opal_ifinit();
    if (rc != OPAL_SUCCESS)
...
int btl_usnic_opal_ifindextomask(int if_index, uint32_t* if_mask, int length)
{
    btl_usnic_opal_if_t* intf;
    int rc = btl_usnic_opal_ifinit();
    if (rc != OPAL_SUCCESS)
...
int btl_usnic_opal_ifindextoflags(int if_index, uint32_t* if_flags)
{
    btl_usnic_opal_if_t* intf;
    int rc = btl_usnic_opal_ifinit();
    if (rc != OPAL_SUCCESS)
...
int btl_usnic_opal_ifindextoname(int if_index, char* if_name, int length)
{
    btl_usnic_opal_if_t *intf;
    int rc = btl_usnic_opal_ifinit();
    if (rc != OPAL_SUCCESS)
...
int btl_usnic_opal_ifkindextoname(int if_kindex, char* if_name, int length)
{
    btl_usnic_opal_if_t *intf;
    int rc = btl_usnic_opal_ifinit();
    if (rc != OPAL_SUCCESS)
...
#define ADDRLEN 100
bool
btl_usnic_opal_ifislocal(const char *hostname)
{
    int ret;
#if OPAL_WANT_IPV6
    ...
    ret = btl_usnic_opal_ifaddrtoname(hostname, addrname, ADDRLEN);
...
static uint32_t parse_dots(char *addr)
{
    ...
    net = BTL_USNIC_OPAL_IF_ASSEMBLE_NETWORK(n[0], n[1], n[2], n[3]);
    opal_argv_free(tuple);
    return net;
}

int
btl_usnic_opal_iftupletoaddr(char *inaddr, uint32_t *net, uint32_t *mask)
{
    char *addr;
    char **tuple;
    int pval;
...
bool btl_usnic_opal_ifisloopback(int if_index)
{
    btl_usnic_opal_if_t* intf;
    int rc = btl_usnic_opal_ifinit();
    if (rc != OPAL_SUCCESS) {
...
int
btl_usnic_opal_ifnametoaddr(const char* if_name, 
...
int
btl_usnic_opal_ifaddrtoname(const char* if_addr, 
...
int
btl_usnic_opal_ifnametoindex(const char* if_name)
{
    return OPAL_ERR_NOT_SUPPORTED;
}

int16_t
btl_usnic_opal_ifnametokindex(const char* if_name)
{
    return OPAL_ERR_NOT_SUPPORTED;
}

int
btl_usnic_opal_ifindextokindex(int if_index)
{
    return OPAL_ERR_NOT_SUPPORTED;
}

int
btl_usnic_opal_ifcount(void)
{
    return OPAL_ERR_NOT_SUPPORTED;
}

int
btl_usnic_opal_ifbegin(void)
{
    return OPAL_ERR_NOT_SUPPORTED;
}

int
btl_usnic_opal_ifnext(int if_index)
{
    return OPAL_ERR_NOT_SUPPORTED;
}

int
btl_usnic_opal_ifindextoname(int if_index, char* if_name, int length)
{
    return OPAL_ERR_NOT_SUPPORTED;
}

int
btl_usnic_opal_ifkindextoname(int kif_index, char* if_name, int length)
{
    return OPAL_ERR_NOT_SUPPORTED;
}

int
btl_usnic_opal_ifindextoaddr(int if_index, struct sockaddr* if_addr, unsigned int length)
{
    return OPAL_ERR_NOT_SUPPORTED;
}

int
btl_usnic_opal_ifindextomask(int if_index, uint32_t* if_addr, int length)
{
    return OPAL_ERR_NOT_SUPPORTED;
}
...
int
btl_usnic_opal_iffinalize(void)
{
    return OPAL_SUCCESS;
}
